...
CREATE TABLE body_types (
    body_type_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE cars (
    ...
    body_type_id     BIGINT       NOT NULL REFERENCES body_types(body_type_id) ON DELETE RESTRICT,
    year             INTEGER,
    ...
    CONSTRAINT chk_cars_year CHECK (year IS NULL OR year BETWEEN 1950 AND 2026),
    ...
    CONSTRAINT chk_cars_price_usd CHECK (price_usd IS NULL OR price_usd > 0),
    CONSTRAINT chk_cars_specs_ranges CHECK (
...
CREATE TABLE car_requests (
    ...
    CONSTRAINT chk_car_requests_year CHECK (year IS NULL OR year BETWEEN 1950 AND 2026),
    CONSTRAINT chk_car_requests_fuel_type CHECK (fuel_type IS NULL OR fuel_type IN ('combustion', 'hybrid', 'electric')),
    CONSTRAINT chk_car_requests_transmission CHECK (transmission IS NULL OR transmission IN ('manual', 'automatic')),
    CONSTRAINT chk_car_requests_price_usd CHECK (price_usd IS NULL OR price_usd > 0),
    CONSTRAINT chk_car_requests_specs_ranges CHECK (
        (horsepower IS NULL OR horsepower BETWEEN 1 AND 2000)
        AND (airbag_count IS NULL OR airbag_count BETWEEN 0 AND 30)
        AND (fuel_consumption IS NULL OR fuel_consumption BETWEEN 0.0 AND 99.9)
        AND (max_speed_kmh IS NULL OR max_speed_kmh BETWEEN 1 AND 600)
...
CREATE TABLE review_reply_likes (
    reply_id   BIGINT    NOT NULL REFERENCES review_replies(reply_id) ON DELETE CASCADE,
    user_id    BIGINT    NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE review_tags (
    ...
    created_at TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT chk_review_tags_sentiment CHECK (sentiment IN ('positive', 'negative'))
);
...
CREATE TABLE review_images (
    ...
    image_data    BYTEA        NOT NULL,
    updated_at    TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT chk_review_images_display_order CHECK (display_order >= 0)
);
...
CREATE TABLE admin_requests (
    ...
    submitted_by_user_id BIGINT       NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    submitter_email      VARCHAR(100),
    motivation           LONGVARCHAR  NOT NULL,
    bio                  LONGVARCHAR  NOT NULL,
    justification        LONGVARCHAR  NOT NULL,
    status               VARCHAR(20)  NOT NULL DEFAULT 'pending',
    created_at           TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE community_post_images (
    ...
    image_data     BYTEA        NOT NULL,
    updated_at     TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT chk_community_post_images_display_order CHECK (display_order >= 0),
    CONSTRAINT uq_community_post_images_order UNIQUE (post_id, display_order)
);
