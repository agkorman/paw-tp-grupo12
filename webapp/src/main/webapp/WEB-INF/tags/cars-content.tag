    ...
    <section class="catalog-section">
            ...
            <c:if test="${showCarRequestCard}">
                <sec:authorize access="isAuthenticated()">
                    <a href="${newCarUrl}" class="car-request-card" data-auth-resume-intent="create-car">
                        ...
                        <strong class="car-request-card-title"><spring:message code="cars.empty.title"/></strong>
                        <span class="car-request-card-copy"><spring:message code="cars.empty.text"/></span>
                        <span class="btn-primary car-request-card-action"><spring:message code="cars.empty.add"/></span>
                    </a>
                </sec:authorize>
                <sec:authorize access="isAnonymous()">
                    <a href="${newCarUrl}"
                       ...
                       data-auth-return-url="/cars/new">
                        ...
                        <strong class="car-request-card-title"><spring:message code="cars.empty.title"/></strong>
                        <span class="car-request-card-copy"><spring:message code="cars.empty.loginText"/></span>
                        <span class="btn-primary car-request-card-action"><spring:message code="common.action.login"/></span>
                    </a>
                </sec:authorize>
            </c:if>
        </div>
        ...
        <c:if test="${not empty totalPages and totalPages > 1}">
            ...
            <c:if test="${not empty criteria}">
                ...
                <c:if test="${not empty criteria.fuelConsumptionMax and not criteria.electricOnly}"><c:set target="${paginationParams}" property="fuelConsumptionMax" value="${criteria.fuelConsumptionMax}"/></c:if>
