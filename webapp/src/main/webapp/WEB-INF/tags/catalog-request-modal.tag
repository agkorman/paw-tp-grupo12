...
<div id="${fn:escapeXml(id)}" class="modal catalog-request-modal" hidden>
    ...
    <section class="modal-dialog catalog-request-dialog" role="dialog" aria-modal="true" aria-labelledby="${fn:escapeXml(titleId)}">
        <header class="modal-header">
            <div>
                <span class="modal-kicker"><spring:message code="${kickerCode}"/></span>
                <h2 id="${fn:escapeXml(titleId)}"><spring:message code="${titleCode}"/></h2>
            </div>
            <button type="button" class="modal-close" data-close-catalog-request-modal aria-label="${fn:escapeXml(closeModalLabel)}">
